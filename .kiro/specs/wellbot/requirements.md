# WellBOT Requirements

## Introduction

WellBOT — это Telegram бот-парсер для отслеживания новых объявлений на белорусских площадках (Kufar.by, Onliner.by, Av.by). Бот предоставляет простой способ получать уведомления о новых объявлениях по заданным пользователем ссылкам с фильтрами.

## Glossary

- **Bot** - Telegram бот WellBOT
...
- **Parser** - компонент системы, который извлекает данные с площадок объявлений
- **Database** - PostgreSQL база данных для хранения пользователей, ссылок и объявлений
- **Platform** - площадка объявлений (Kufar, Onliner, Av)
...
2. THE Bot SHALL сохранить данные пользователя (telegram_id, username) в базе данных
...
3. WHEN User отправляет URL, THE Bot SHALL определить платформу (Kufar, Onliner, Av) по домену
4. THE Bot SHALL проверить корректность ссылки для определенной платформы
5. IF ссылка корректна, THEN THE Bot SHALL сохранить ссылку в базе данных и начать отслеживание
...
2. WHEN Parser обнаруживает новое объявление, THE Parser SHALL сохранить его в базе данных
3. THE Parser SHALL извлекать следующие данные: заголовок, описание, цену, фото, ссылку на объявление, местоположение
4. THE Parser SHALL обрабатывать разные типы объявлений (недвижимость, авто, другое)
5. IF парсинг ссылки завершается ошибкой более 5 раз подряд, THEN THE Bot SHALL пометить ссылку как неактивную и уведомить пользователя
...
2. THE Bot SHALL отправлять уведомление с фото, заголовком, ценой, адресом и ссылкой на объявление
3. IF у объявления нет фото, THEN THE Bot SHALL отправить текстовое сообщение
4. THE Bot SHALL группировать уведомления (не более 5 сообщений в цикл парсинга)
...
1. WHEN User выбирает ссылку из списка, THE Bot SHALL отобразить информацию о ссылке (URL, платформу)
2. THE Bot SHALL предоставить кнопку "Удалить"
3. WHEN User удаляет ссылку, THE Bot SHALL удалить ссылку из базы данных
...
4. THE Bot SHALL автоматически переподключаться к базе данных при потере соединения
...
2. THE Bot SHALL использовать переменные окружения для конфигурации (TELEGRAM_BOT_TOKEN, DATABASE_URL, YANDEX_MAPS_API_KEY)
3. THE Bot SHALL автоматически создавать необходимые таблицы в базе данных при первом запуске
